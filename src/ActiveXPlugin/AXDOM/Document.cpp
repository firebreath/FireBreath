...
FB::DOM::WindowPtr Document::getWindow()
{
    ...
    FB::JSObjectPtr api(new IDispatchAPI(htmlWin.p, as_ActiveXBrowserHost(m_element->host)));
...
FB::DOM::ElementPtr Document::getElementById(const std::string& elem_id)
{
    ...
    FB::JSObjectPtr ptr(new IDispatchAPI(disp.p, as_ActiveXBrowserHost(this->m_element->host)));
...
std::vector<FB::DOM::ElementPtr> Document::getElementsByTagName(const std::string& tagName)
{
    ...
    if (SUCCEEDED(list->get_length(&length))) {
        for (long i = 0; i < length; i++) {
            ...
            FB::JSObjectPtr obj(new IDispatchAPI(dispObj.p, as_ActiveXBrowserHost(this->m_element->host)));
