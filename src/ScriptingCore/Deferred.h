...
/// type T
///
/// FB::Promise objects are patterned after the Javascript Promise pattern and
/// are a method for
/// returning a value which will *eventually* resolve to a value, or reject with
/// an exception
/// in case of an error.  To create a FB::Promise object, first create a
/// FB::Deferred<T> which
...
///
/// FB::Promise objects can be moved or copied; all copies share state, are
/// controlled by the
/// same FB::Deferred object(s), and thus resolve or reject together.
////////////////////////////////////////////////////////////////////////////////////////////////////
...
///
/// FB::Deferred objects are only useful as the control mechanism for a
/// FB::Promise object.
/// To create a new Promise, first create a FB::Deferred and then return the
/// promise for it. Retain
/// a reference to the Deferred or to a copy of it and call resolve or reject
/// when the result
/// of the promise is known.
///
/// A FB::Deferred object can be moved or copied; all copies share state and
/// retain control over
/// any associated FB::Promise objects.
////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename T> class Deferred final {
...
public:
  /// @brief Instantiates a Deferred with a Promise which is already resolved to
  /// v
  ...
  Deferred(Deferred<T> &&rh)
      : m_data(std::move(rh.m_data)) {} // Move constructor
...
private:
  ...
  friend Promise<Uout>
  _doPromiseThen(const Promise<T> &inPromise, std::function<Uout(T)> cbSuccess,
                 std::function<Uout(std::exception_ptr)> cbFail);
...
public:
  ...
  ///
  /// Note that this will not invalidate any reject or resolve handlers unless
  ...
  // available only when creating a variantDeferred, allows forcing the type in
  // the variant.
  ...
  template <typename U, typename V = T,
            typename std::enable_if<std::is_same<FB::variant, V>::value,
                                    int>::type = 0>
  ...
  ///
  /// This relies on FB::variant::convert_cast and can only be used to convert
  /// types
  /// which would succeed with a convert_cast. If an exception is thrown during
  /// conversion
  /// the returned Promise object will be rejected with that exception
  ///
  /// @return a Promise of the new type which will resolve after the original
  /// Promise resolves and a FB::variant::convert_cast succeeds
  ...
  /// Promise<Uout> which
  ///        resolves to the value returned from those handlers. (The handlers
  /// must each return
  ///        this type)
  ///
  ...
  /// cbSuccess or cbFail can be nullptr if only one is desired. Often a lambda
  /// expression may
  /// be appropriate for one or both of these Callable types
  ///
  /// @param cbSuccess nullptr or any Callable target accepting one parameter of
  /// type T and returning a value of type Uout
  /// @param cbFail    nullptr or any Callable target accepting one parameter of
  /// type std::exception and returning a value of type Uout
  ///
  /// @see http://en.cppreference.com/w/cpp/utility/functional/function
  template <typename Uout>
  Promise<Uout> then(std::function<Uout(T)> cbSuccess,
                     std::function<Uout(std::exception_ptr)> cbFail =
                         nullptr) const {
  ...
  /// @brief Accepts a Success handler and a Fail handler, returns a new
  /// Promise<Uout> which
  ///        chains to a Promise<Uout> returned from those handlers. (The
  /// handlers must each return
  ///        a Promise of this type)
  ///
  /// This is particularly useful when you need several asynchronous commands to
  /// execute one after another
  /// and then want to deal with the result in one place. Your handler will be
  /// Called with the result of
  /// this Promise and it can then return a Promise of a different type (Uout).
  /// The Promise returned
  /// by this function will resolve or reject when that Promise does. These can
  /// be chained together
  /// in some very powerful ways.  Often a lambad expression may be appropriate
  /// for one or both
  /// of these Callable types
  ///
  /// @param cbSuccess nullptr or any Callable target accepting one parameter of
  /// type T and returning a Promise of type Uout
  /// @param cbFail    nullptr or any Callable target accepting one parameter of
  /// type std::exception and returning a Promise of type Uout
  ///
  /// @see http://en.cppreference.com/w/cpp/utility/functional/function
  template <typename Uout>
  Promise<Uout> thenPipe(std::function<Promise<Uout>(T)> cbSuccess,
  ...
  /// @brief registers a Callable handler to be called if/when the Promise
  /// resolves
  ///
  /// Optionally accepts a second parameter with a failure handler
  ///
  /// @param cbSuccess  nullptr or any Callable target accepting one parameters
  /// of type T and returning void
  /// @param cbFail     nullptr or any Callable target accepting one parameter
  /// of type std::exception and returning void
  const Promise<T> &done(typename Deferred<T>::Callback cbSuccess,
                         typename Deferred<T>::ErrCallback cbFail =
                             nullptr) const {
    if (!m_data) {
  ...
  /// @brief registers a Callable handler to be called if/when the Promise is
  /// rejected
  ///
  /// @param cbFail     nullptr or any Callable target accepting one parameter
  /// of type std::exception and returning void
...
/// @brief Template specialization of Promise for void type, resolve state has
/// no value
...
private:
  ...
  template <typename Uout>
  friend Promise<Uout>
  _doPromiseThen(const Promise<void> &inPromise,
                 std::function<Uout(void)> cbSuccess,
                 std::function<Uout(std::exception_ptr)> cbFail);
...
public:
  ...
  /// @brief Assigns rh to this Promise<void>, assuming all shared state from rh
  /// and discarding any current state
  ///
  /// Note that this will not invalidate any reject or resolve handlers unless
  /// this is the last Promise<void>
  ...
  template <typename U> Promise(const Promise<U> &rh) {
    Deferred<void> dfd;
  ...
  ///
  /// Since Promise<void> does not have a value, this will result in a Promise
  /// which
  /// returns the default value of whatever type. It is included more for
  /// completeness than
  ...
  /// This is very useful when you know a command might fail but want to
  /// properly handle it
  /// or when you want to provide a value for when the Promise<void> resolves.
  /// Either
  /// cbSuccess or cbFail can be nullptr if only one is desired. Often a lambda
  /// expression may
  /// be appropriate for one or both of these Callable types
  ///
  /// @param cbSuccess nullptr or any Callable target accepting no parameters
  /// and returning a value of type Uout
  /// @param cbFail    nullptr or any Callable target accepting one parameter of
  /// type std::exception and returning a value of type Uout
  ///
  /// @see http://en.cppreference.com/w/cpp/utility/functional/function
  template <typename Uout>
  Promise<Uout> then(std::function<Uout()> cbSuccess,
                     std::function<Uout(std::exception_ptr)> cbFail =
                         nullptr) const {
    if (!m_data) {
      return Promise<Uout>::rejected(std::runtime_error("Promise invalid"));
  ...
  /// and then want to deal with the result in one place. Your handler will be
  /// Called and
  /// it can then return a Promise of a different type (Uout). The Promise
  /// returned
  /// by this function will resolve or reject when that Promise does. These can
  /// be chained together
  /// in some very powerful ways.  Often a lambad expression may be appropriate
  ...
  ///
  /// @param cbSuccess nullptr or any Callable target accepting no parameters
  /// and returning a Promise of type Uout
  /// @param cbFail    nullptr or any Callable target accepting one parameter of
  /// type std::exception and returning a Promise of type Uout
  ///
  /// @see http://en.cppreference.com/w/cpp/utility/functional/function
  template <typename Uout>
  Promise<Uout> thenPipe(std::function<Promise<Uout>()> cbSuccess,
                         std::function<Promise<Uout>(std::exception_ptr)> cbFail =
                             nullptr) const {
    if (!m_data) {
      return Promise<Uout>::rejected(std::runtime_error("Promise invalid"));
  ...
  /// @brief registers a Callable handler to be called if/when the Promise<void>
  /// resolves
  ///
  /// Optionally accepts a second parameter with a failure handler
  ///
  ...
  /// @param cbFail     nullptr or any Callable target accepting one parameter
  /// of type std::exception and returning void
  const Promise<void> &done(Deferred<void>::Callback cbSuccess,
                            Deferred<void>::ErrCallback cbFail =
                                nullptr) const {
    if (!m_data) {
      throw std::runtime_error("Promise invalid");
    }
