...
axmain.cpp/.h - This contains the root object used by ATL to create
    new activex controls, as well as the AxIdMap used to map member
    names (methods, properties, and events) to DISPID values.  You
    will not use any of this directly in a plugin.
...
dlldata.c, dlldatax.c, dlldatax.h - Yeah, don't worry about these.
    They are generated by ATL to do some minor things that aren't that
    important to what we're doing. (READ: I don't remember exactly
    what they do, and I don't feel like reading through to find out)

FireBreathWin.cpp - This contains some important DLL entrypoints for
    the COM control: DllCanUnloadNow, DllGetClassObject,
