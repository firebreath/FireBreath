...
namespace FB {
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    /// @brief  Asynchronous return value which will reject or resolve to a value of type T
    ///
    ...
    ///
    /// FB::Promise objects can be moved or copied; all copies share state, are controlled by the
    ...
    /// a reference to the Deferred or to a copy of it and call resolve or reject when the result
    /// of the promise is known.
    ///
    /// A FB::Deferred object can be moved or copied; all copies share state and retain control over
    ...
    public:
        ...
        /// @brief Destroys the Deferred.  Note that this doesn't clean up Promise objects unless this is the last copy
        ~Deferred() { }
        ...
        /// @brief Returns a FB::Promise<T> object controlled by this FB::Deferred object
        Promise<T> promise() const {
            return Promise<T>(m_data);
        }

        /// @brief invalidates this Deferred; if the object is still pending, reject it
    ...
    /// @brief Template specialization of Deferred for type void; works slightly different from normal Deferred in that a Resolved state has no value
    template <>
    ...
    private:
        ...
        StateDataPtr m_data;
    public:
        /// @brief Instantiates a Deferred<void> with a Promise<void> which is resolved if v is true, else pending
        Deferred(bool v) : m_data(std::make_shared<StateData>(v)) {}
        /// @brief Instantiates a Deferred<void> with a Promise which is already rejected with e
        Deferred(std::exception e) : m_data(std::make_shared<StateData>(e)) {}
        ...
        /// @brief Destroys the Deferred<void>.  Note that this doesn't clean up Promise<void> objects unless this is the last copy
        ~Deferred() { }

        /// @brief Copies another Deferred<void> into this one, assuming its shared state
        Deferred<void>& operator=(const Deferred<void>& rh) {
            m_data = rh.m_data;
            return *this;
        }
        /// @brief Moves another Deferred<void> into this one, assuming its shared state
        Deferred<void>& operator=(const Deferred<void>&& rh) {
            m_data = std::move(rh.m_data);
            return *this;
        }

        /// @brief Returns a FB::Promise<void> object controlled by this FB::Deferred object
        Promise<void> promise() const;

        /// @brief invalidates this Deferred; if the object is still pending, reject it
        void invalidate() const;

        /// @brief All associated Promise<void> objects with resolve or reject along with v
    ...
    _doPromiseThen(const Promise<Uin>& inP, std::function<void(Uin)> cbSuccess, std::function<void(std::exception)> cbFail);
    ...
    private:
        ...
        friend Promise<Uout> _doPromiseThen(const Promise<T>& inPromise, std::function<Uout(T)> cbSuccess, std::function<Uout(std::exception)> cbFail);

    public:
        /// @brief Creates an invalid Promise; useful only if you plan to use the assignment operator later
        Promise() {}
        ...
        /// @brief The only valid way to create a Promise without a Deferred, creates a pre-resolved Promise
        Promise(T v) {
            Deferred<T> dfd{ v };
            m_data = dfd.promise().m_data;
        }

        /// @brief Assigns rh to this Promise, assuming all shared state from rh and discarding any current state
        ///
        ...
        /// @brief Accepts a Success handler and a Fail handler, returns a new Promise<Uout> which
        ///        resolves to the value returned from those handlers. (The handlers must each return
        ///        this type)
        ///
        ...
        template <typename Uout>
        Promise<Uout> then(std::function<Uout(T)> cbSuccess, std::function<Uout(std::exception)> cbFail = nullptr) const {
            return _doPromiseThen<Uout, T>(*this, cbSuccess, cbFail);
        }

        /// @brief Accepts a Success handler and a Fail handler, returns a new Promise<Uout> which
        ///        chains to a Promise<Uout> returned from those handlers. (The handlers must each return
        ...
        /// @see http://en.cppreference.com/w/cpp/utility/functional/function
        template <typename Uout>
        Promise<Uout> thenPipe(std::function<Promise<Uout>(T)> cbSuccess, std::function<Promise<Uout>(std::exception)> cbFail = nullptr) const {
    ...
    private:
        ...
        template <typename Uout>
        friend Promise<Uout> _doPromiseThen(const Promise<void>& inPromise, std::function<Uout(void)> cbSuccess, std::function<Uout(std::exception)> cbFail);

    public:
        /// @brief Creates an invalid Promise<void>; useful only if you plan to use the assignment operator later
        Promise() {}
        Promise(Deferred<void>::StateDataPtr data) : m_data(data) {}
        /// @brief Creates a Promise<void? object using shared state from Promise rh (move)
        Promise(Promise&& rh) : m_data(std::move(rh.m_data)) {} // Move constructor
        /// @brief Creates a Promise<void> object using shared state from Promise rh (copy)
        Promise(const Promise& rh) : m_data(rh.m_data) {} // Copy constructor
        /// @brief The only valid way to create a Promise<void> without a Deferred<void>, creates a Promise<void> which is resolved if v is true, else pending
        Promise(bool v) {
            Deferred<void> dfd{ v };
            m_data = dfd.promise().m_data;
        }
        ...
        ///
        /// This is very useful when you know a command might fail but want to properly handle it
        /// or when you want to provide a value for when the Promise<void> resolves. Either
        /// cbSuccess or cbFail can be nullptr if only one is desired. Often a lambda expression may
        /// be appropriate for one or both of these Callable types
        ///
    ...
    _doPromiseThen(const Promise<Uin>& inP,
        ...
        if (!inP.isValid()) { return Promise<Uout>::rejected(std::runtime_error("Promise invalid")); }
        Deferred<Uout> dfd;
